...
instance FromJSON Base64Binary where
    parseJSON = withText "Base64Binary" $ pure . Base64Binary
...
instance FromJSON Canonical where
    parseJSON = withText "Canonical" $ pure . Canonical
...
instance FromJSON Code where
    parseJSON = withText "Code" $ pure . Code
...
instance FromJSON Decimal where
    parseJSON = withScientific "Decimal" $ pure . Decimal
...
instance FromJSON Instant where
    parseJSON v = Instant <$> parseJSON v
...
instance FromJSON Integer' where
    parseJSON v = Integer' <$> parseJSON v
...
instance FromJSON Markdown where
    parseJSON = withText "Markdown" $ pure . Markdown
...
instance FromJSON Oid where
    parseJSON = withText "Oid" $ pure . Oid
...
instance FromJSON PositiveInt where
    parseJSON v = do
        ...
        if n >= 1
            ...
            else fail "PositiveInt must be >= 1"
...
instance FromJSON FhirString where
    parseJSON = withText "FhirString" $ pure . FhirString
...
instance FromJSON UnsignedInt where
    parseJSON v = UnsignedInt <$> parseJSON v
